...
export default function SearchModal({ isOpen, onClose, onArtworkClick }: SearchModalProps) {
            ...
            >
              {filteredProducts.length > 0 ? (
                <div className="divide-y divide-white/10">
                  {/* Show artist page option if searching for an artist */}
                  {searchQuery && filteredProducts.length > 0 && (() => {
                    const artistName = filteredProducts[0].artist;
                    const matchesArtist = artistName.toLowerCase().includes(searchQuery.toLowerCase());
                    if (matchesArtist) {
                      ...
                      return (
                        <Link
                          ...
                          onClick={onClose}
                        >
                          <div className="flex gap-4 p-4 bg-purple-600/20 hover:bg-purple-600/30 transition-colors cursor-pointer border-b-2 border-purple-500">
                            <div className="w-16 h-16 rounded-lg flex-shrink-0 bg-purple-600/40 flex items-center justify-center">
                              <span className="text-2xl font-bold text-white">{artistName.charAt(0)}</span>
                            </div>
                            <div className="flex-1 min-w-0">
                              <h3 className="font-semibold text-white">View {artistName}'s Artist Page</h3>
                              <p className="text-sm text-purple-300">See all artworks by this artist</p>
                              <div className="flex items-center gap-2 mt-1">
                                <span className="text-xs text-purple-400 uppercase">Artist Collection</span>
                              </div>
                            </div>
                          </div>
                        </Link>
                  ...
                  {filteredProducts.slice(0, 10).map((product) => (
                    <div
                      key={product.id}
                      onClick={() => {
                        // Open lightbox if callback is provided, otherwise navigate
                        if (onArtworkClick) {
                          onArtworkClick(product);
                          onClose();
                        } else {
                          const artistSlug = product.artist.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
                          setLocation(`/artist/${artistSlug}?artwork=${product.id}`);
                          onClose();
                        }
                      }}
                      className="flex gap-4 p-4 hover:bg-white/10 transition-colors cursor-pointer"
                    >
                      <img
                        src={product.image}
                        alt={product.title}
